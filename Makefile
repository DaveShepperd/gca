...
HARD_PARAMS_FLAGS= -DNO_SIG -DNO_LONG_DOUBLE -m32 -g

# Directory where sources are, from where we are.
srcdir = .
# Directory in which to put the executable for the command `gcc'
#bindir = $(prefix)/u/local
#bindir = $(prefix)/usr/local/gnu/IRIX/asap/bin
bindir = $(prefix)/usr/local/asap-elf/bin
# Directory in which to put the subprograms used by the compiler.
#libdir = $(prefix)/u/local/lib
#libdir = $(prefix)/usr/local/gnu/IRIX/asap/lib
libdir = $(prefix)/usr/local/asap-elf/lib
# Directory in which to put man pages.
#mandir = $(prefix)/usr/local/man/man1
#mandir = $(prefix)/usr/local/gnu/IRIX/asap/man/man1
mandir = $(prefix)/usr/local/asap-elf/man/man1
...
# since the one that comes with the system is good for POSIX.
 RANLIB = :
# MEA changed below because 1.40's cc1 barfed on 1.37's cpp output
#CC = gcc
#OLDCC = rcc
OLDCC = gcc
...
#all-libconvert: config.status gnulib gca cc1 cpp float.h # cc1plus
...
stamp-gnulib2: gnulib2.c gnulib cc1 gca cpp $(CONFIG_H)
	for name in $(LIB2FUNCS); \
	do \
	  echo $${name}; \
	  ./gca -B./ -fstrength-reduce -O $(INCLUDES) $(GNULIB2_CFLAGS) -c -DL$${name} $(srcdir)/gnulib2.c -o $${name}.o; \
	  $(AR) rc gnulib $${name}.o; \
	  rm -f $${name}.o; \
	done
...
# Delete the temp files made in the course of building gnulib.
	...
	-rm -f core* float.h hard-params gca make.log
...
install: all $(USER_H) float.h gvarargs.h gstdarg.h gcc.1
	...
	$(INSTALL) $(srcdir)/gcc.1 $(mandir)/gca.$(manext)
	-chmod a-x $(mandir)/gca.$(manext)
