...
{
#ifdef	CWD_BUFLEN
    char curr_dir_buf[CWD_BUFLEN+2],*curr_dir;
#endif
  asmfile = asm_file;
...
#ifdef CWD_BUFLEN
  if ( *input_file_name != '/' ) {
    /* relative file name, append current working directory */
    curr_dir = getcwd(curr_dir_buf,CWD_BUFLEN);
    if ( curr_dir == 0 ) curr_dir = "";
    else strcat(curr_dir,"/");
  } else curr_dir = "";
  fprintf (asmfile,
	   "\t.stabs \"%s%s\",%d,0,0,Ltext\nLtext:\n",
	   curr_dir,input_file_name, N_SO);
#else
  fprintf (asmfile,
	   "\t.stabs \"%s\",%d,0,0,Ltext\nLtext:\n",
