...
static char *br_kluge;
static int brk_len;

char *
moto_jump(string,insn)
char *string;
rtx insn;
{
    int n_insns,len;
    ...
    if ( !TARGET_BRAS ) return string;
    code = GET_CODE(insn);
#if (0)
    fprintf(stderr,"\nmoto_jump(\"%s\",",string);
    debug_rtx(insn);
#endif
    if ( code != JUMP_INSN ) return string;
    if ( *string != 'j' ) return string;
    target = JUMP_LABEL(insn);
#if (0)
    fprintf(stderr,"JUMP_LABEL = (UID %d) .L%d\n",
      target->fld[0].rtint,
	...
	switch (code) {
		...
		break;
	    case JUMP_INSN :
		len += 4;	/* max jump (really branch) insn */
		++n_insns;
		break;
	}
    } while ( iptr && iptr != target && len < 128 );
    if ( target == iptr && len < 128 ) {
	if ( !br_kluge ) {
	    /* no kluge buffer allocated yet */
	    brk_len = strlen(string) + 3;
	    br_kluge = (char *) malloc(brk_len);
	} else if ( brk_len < (strlen(string) + 3) ) {
	    /* current buffer too small */
	    brk_len = strlen(string) + 3;
	    br_kluge = (char *) realloc(br_kluge,brk_len);
	}
	if ( br_kluge ) {
	    /* if we have room, build a modified copy of the template
	    *  Otherwise, shine it on.
	    */
	    char *src,*dst;
	    src = string+1;
	    dst = br_kluge;
	    /* copy Bxx part of jbxx %ln */
	    *dst++ = *src++;
	    *dst++ = *src++;
	    *dst++ = *src++;
	    *dst++ = '.';	/* "inject" ".s" */
	    *dst++ = 's';
	    strcpy(dst,src);	/* copy rest of template */
	    fprintf(asm_out_file,
	      "\n* Target (.L%d) found within %d insns, %d bytes\n",
	      CODE_LABEL_NUMBER(target),n_insns,len);
	    return br_kluge;
	}
    }
    else {
