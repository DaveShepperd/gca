     ...
     int nopeepholes;
{
  ...
  switch (GET_CODE (insn))
	...
	if (! constrain_operands (insn_code_number)) {
	  fprintf(stderr,
	    "failed constrain_operands(%d) on insn:\n",insn_code_number);
	  debug_rtx(insn);
	  fflush(stderr);
	  abort ();
	}
	...
	if (template == 0)
	  {
		...
		if (PREV_INSN (insn) != last_ignored_compare) {
		  fprintf(stderr,"Jump insn following deleted test!\n");
		  abort ();
		}
		new_block = 0;
     ...
     rtx x;
{
 ...
 restart:
  switch (GET_CODE (x))
    ...
    default:
      fprintf(stderr,"output_addr_const() can't handle:\n");
      debug_rtx(x);
